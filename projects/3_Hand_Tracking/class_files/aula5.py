...
def processa_desenho(img, info_dedos_mao1, info_dedos_mao2, indicador_coords):
    global cor_pincel, img_quadro, espessura_pincel, x_quadro, y_quadro
    indicador_x, indicador_y, indicador_z = indicador_coords
    ...
    cv2.circle(
        img, (indicador_x, indicador_y), espessura_pincel, cor_pincel, cv2.FILLED
    )
...
while True:
    ...
    if len(todas_maos) == 2:
        ...
        img = processa_desenho(
            img, info_dedos_mao1, info_dedos_mao2, todas_maos[0]["coordenadas"][8]
        )
...
cv2.imwrite(os.path.join(results_dir, "quadro.png"), img_quadro)

"""
ideia:
> se quiser instruções, os dois polegares
em uma caixa (linha) abaixo
então enquanto estiver com os 10 dedos levantados, mostrar todas as instruções
> para escrever, use sua mão direita. apague com dedo mindinho
> para abrir aplicativos, use sua mão esquerda. com o indicador, abre o word, com o indicador e o meio abre o excel...
> para desenha, use as duas mãos (a primeira mão que colocar na tela, selecione a cor e desenhe com a mão segunda. se 1 dedo levantada, cor verde, se... e se fechar a mão, apaga o desenho. a distancia da sua mao direita muda a expessura do pincel)
- melhorar explicaçoes e tudo que vai aqui
"""
